...
func doRunImportCustomer() {
	...
	irr := repositories.NewCustomerRepo(db)
	...
	runCustomerETL(ctx, tenant.Id, irr, oldDb)
}
...
func runCustomerETL(ctx context.Context, tenantId uint64, irr *repositories.CustomerRepo, oldDb *sql.DB) {
	customers, err := ListAllCustomers(oldDb)
	if err != nil {
		log.Fatal(err)
	}
	for _, oir := range customers {
		insertCustomerETL(ctx, tenantId, irr, oir)
	}
}

func insertCustomerETL(ctx context.Context, tid uint64, irr *repositories.CustomerRepo, oir *OldUCustomer) {
	// var state int8 = 1
