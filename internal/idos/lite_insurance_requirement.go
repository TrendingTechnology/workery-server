...
func NewLiteInsuranceRequirementListResponseIDO(arr []*models.LiteInsuranceRequirement, count uint64) *LiteInsuranceRequirementListResponseIDO {
	// Calculate next id.
	var nextId uint64
	if len(arr) > 0 {
	...
	res := &LiteInsuranceRequirementListResponseIDO{ // Return through HTTP.
		Count:   count,
		Results: arr,
		NextId:  nextId,
	}
