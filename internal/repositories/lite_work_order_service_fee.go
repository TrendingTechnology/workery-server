...
func NewLiteWorkOrderServiceFeeRepo(db *sql.DB) *LiteWorkOrderServiceFeeRepo {
	return &LiteWorkOrderServiceFeeRepo{
		db: db,
	}
}

func (s *LiteWorkOrderServiceFeeRepo) queryRowsWithFilter(ctx context.Context, query string, f *models.LiteWorkOrderServiceFeeFilter) (*sql.Rows, error) {
	...
	// log.Println("LiteWorkOrderServiceFeeRepo | query:", query, "\n")
	// log.Println("LiteWorkOrderServiceFeeRepo | filterValues:", filterValues, "\n")
...
func (s *LiteWorkOrderServiceFeeRepo) ListByFilter(ctx context.Context, filter *models.LiteWorkOrderServiceFeeFilter) ([]*models.LiteWorkOrderServiceFee, error) {
	...
	var arr []*models.LiteWorkOrderServiceFee
	defer rows.Close()
	for rows.Next() {
		m := new(models.LiteWorkOrderServiceFee)
		err := rows.Scan(
			&m.Id,
			&m.TenantId,
			&m.Title,
