...
func (s *LiteTagRepo) queryRowsWithFilter(ctx context.Context, query string, f *models.LiteTagFilter) (*sql.Rows, error) {
	...
	// log.Println("LiteTagRepo | query:", query, "\n")
	// log.Println("LiteTagRepo | filterValues:", filterValues, "\n")

	return s.db.QueryContext(ctx, query, filterValues...)
}

func (s *LiteTagRepo) ListByFilter(ctx context.Context, filter *models.LiteTagFilter) ([]*models.LiteTag, error) {
	...
	var arr []*models.LiteTag
	defer rows.Close()
	for rows.Next() {
		m := new(models.LiteTag)
		err := rows.Scan(
			&m.Id,
			&m.TenantId,
			&m.Text,
