...
func NewLiteInsuranceRequirementRepo(db *sql.DB) *LiteInsuranceRequirementRepo {
	return &LiteInsuranceRequirementRepo{
		db: db,
	}
}

func (s *LiteInsuranceRequirementRepo) queryRowsWithFilter(ctx context.Context, query string, f *models.LiteInsuranceRequirementFilter) (*sql.Rows, error) {
	...
	// log.Println("LiteInsuranceRequirementRepo | query:", query, "\n")
	// log.Println("LiteInsuranceRequirementRepo | filterValues:", filterValues, "\n")
...
func (s *LiteInsuranceRequirementRepo) ListByFilter(ctx context.Context, filter *models.LiteInsuranceRequirementFilter) ([]*models.LiteInsuranceRequirement, error) {
	...
	var arr []*models.LiteInsuranceRequirement
	defer rows.Close()
	for rows.Next() {
		m := new(models.LiteInsuranceRequirement)
		err := rows.Scan(
			&m.Id,
			&m.TenantId,
			&m.Text,
