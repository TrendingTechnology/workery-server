...
func NewLiteSkillSetRepo(db *sql.DB) *LiteSkillSetRepo {
	return &LiteSkillSetRepo{
		db: db,
	}
}

func (s *LiteSkillSetRepo) queryRowsWithFilter(ctx context.Context, query string, f *models.LiteSkillSetFilter) (*sql.Rows, error) {
	...
	// log.Println("LiteSkillSetRepo | query:", query, "\n")
	// log.Println("LiteSkillSetRepo | filterValues:", filterValues, "\n")
...
func (s *LiteSkillSetRepo) ListByFilter(ctx context.Context, filter *models.LiteSkillSetFilter) ([]*models.LiteSkillSet, error) {
	...
	var arr []*models.LiteSkillSet
	defer rows.Close()
	for rows.Next() {
		m := new(models.LiteSkillSet)
		err := rows.Scan(
			&m.Id,
			&m.TenantId,
			&m.Category,
